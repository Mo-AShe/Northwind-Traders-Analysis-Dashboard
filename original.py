...
#----------------
    ...
    html.Div([
        html.H4('Sales trends over time',
        style={
            'textAlign': 'center',
...
html.Div(style={
                ...
                'padding': '10px'
            },children=[html.H5('Detailed data', style={
                'textAlign': 'center',
    ...
    data=merged_ship_name.to_dict('records'),
    page_size=10,
    style_table={'marginTop': '0px', 'overflowX': 'auto'},
...
,
 ...
 html.Div(
    [
        'All rights reserved 2025© Northwind Traders',
        html.Br(),
        '📞 tel: +20 xxxxxxxxxxx',
        html.Br(),
         html.A(
            '🌐 Website: Maven Analytics',
            href='https://mavenanalytics.io/challenges/maven-northwind-challenge/88950a50-85b8-4f63-94be-cb3485024306',
            style={'color': 'rgb(0, 0, 0)', 'textDecoration': 'none'}
        )
    ],
    style={
        'color': 'rgb(0, 0, 0)',
        'backgroundColor': 'rgb(206, 255, 254)',
        'marginBottom': '0px',
        'padding': '5px 10px',  
        'height': 'auto',  
        'lineHeight': '1.2em'
...
@app.callback(
    ...
    Output('max-p', 'children'),
    Output('min-p', 'children'),
    Output('title', 'children'),
    Input('per-list', 'value'))
#----------------------------
def update_graph(period):
    if period == 'year':
        figure=px.line(year_sales, x='orderDate', y='Sales')
        max_info=f"Maximum Year:{max_sales_date_year} , Revenue: {max_sales_value_year}$"
        min_info=f"Minumum Year:{min_sales_date_year} , Revenue: {min_sales_value_year}$" 
        title='Year Sales Trends'
    elif period == 'month':
        figure=px.line(month_sales, x='orderDate', y='Sales')
        max_info=f"Maximum Month:{max_sales_date_month} , Revenue: {max_sales_value_month}$"
        min_info=f"Minumum Month:{min_sales_date_month} , Revenue: {min_sales_value_month}$"
        title='Month Sales Trends'
    else:
        figure=px.line(days_sales, x='orderDate', y='Sales')
        max_info=f"Maximum Day:{max_sales_date_day} , Revenue: {max_sales_value_day}$", html.Br(),
        min_info=f"Minumum Day:{min_sales_date_day} , Revenue: {min_sales_value_day}$"
        title='Day Sales Trends'
    

    return figure, max_info,min_info,title
#------------------------------------------
